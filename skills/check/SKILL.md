...
Activate when the user's message starts with "Implement the following plan", followed by a plan body, or links to a `/think` output.
...
Activate when the user mentions: issue, PR, "review all", triage, "batch", or "". Skip the diff flow and run this instead.
...
3. Open with `@<login>` and **at most one** short thanks (``, `thank you for the report`, etc.). Do **not** add closing thanks stacks (``, `Thanks again`, long courtesy endings).
...
**Triage roles (the state machine).** Every triaged item carries exactly one **category** role (`bug` or `enhancement`) and one **state** role: `needs-triage` (maintainer must evaluate), `needs-info` (waiting on the reporter), `ready-for-agent` (fully specified, an agent can take it), `ready-for-human` (needs human judgement, external access, or manual testing), or `wontfix`. An unlabelled item goes to `needs-triage` first; from there to any of the others; `needs-info` returns to `needs-triage` once the reporter replies. Conflicting state roles are a stop: flag it and ask the maintainer before anything else. A PR is an issue with code attached, so it runs the same machine, read against the diff.

**Two checks before recommending anything.**

1. **Redundancy.** Search the codebase for an existing implementation by *domain concept*, not by the request's wording, and report where you looked. Found means this is an already-implemented `wontfix`: point to where it lives, and do not write it to `.out-of-scope/`.
2. **Prior rejection.** Read every file in `.out-of-scope/` and surface anything that resembles this request. Matching is by concept, so "night theme" matches `dark-mode.md`. Surface it rather than re-litigating: "we rejected this before because X, do you still feel the same way?"

**The `.out-of-scope/` knowledge base.** One markdown file per rejected *concept* (not per issue), kebab-cased (`dark-mode.md`, `plugin-system.md`), written like a short design document: what is out of scope, why (project scope, technical constraint, or strategic choice, never "we're too busy", which is a deferral rather than a rejection), and a "Prior requests" list of the issues that asked for it. Write here only when an **enhancement** is rejected as `wontfix`; append to the existing file when one matches. If the maintainer changes their mind, delete the file and let the new issue run normal triage.

**Resuming.** Where prior triage notes exist, read them, check whether the reporter answered the outstanding questions, and present an updated picture. Never re-ask a resolved question.

**Disclosure.** Every comment or issue posted to a public tracker during triage starts with `> *This was generated by AI during triage.*`
...
Activate when the user asks "is this worth a new release", or similar.
