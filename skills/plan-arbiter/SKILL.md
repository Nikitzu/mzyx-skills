---
name: plan-arbiter
description: "Compares competing plans from two or more agents and returns one executable direction as a decision memo. Use when separate agents produced separate strategies for the same work. Not for writing the plan itself (use writing-plans) or a 6-month strategic bet (use synod)."
when_to_use: "compare these plans, which plan is better, codex vs claude plan, merge the best parts, arbitrate between approaches, pick an execution path"
