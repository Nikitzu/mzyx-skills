---
name: visual-recap
description: "Turns a PR, branch, commit, or diff into an interactive visual recap — file maps, diagrams, API and schema summaries, annotated diffs, and review notes — written as local MDX. Use for large, multi-file, or contract-changing changes before line-by-line review. Not for small obvious diffs."
when_to_use: "recap this PR, what did this branch change, visual recap, review this diff at a higher altitude, show me the shape of this change"

metadata:
  version: "0.1.0"
  upstream: "BuilderIO/skills"
---
...
## Mode Is Pinned To Local Files — Read This First

This fork of the skill defaults to **local-files privacy mode**, not the hosted
Agent-Native Plan database. Recap content — diff excerpts, file names, schema and
API shapes, and code — must not be uploaded to `plan.agent-native.com`. Work
repositories are the reason: their diffs are not ours to send to a third-party
processor.

So the default deliverable is a local MDX folder, previewed on a localhost
bridge. Read `references/local-files.md` and follow it. Write the recap to
`.agent-native/plans/<slug>/` (repo-ignored) unless the user asks for it in
source control, in which case use `plans/<slug>/`. Treat `<plan-dir>/.plan-url`
as a token file and never commit it.

Publish to a hosted Plan origin ONLY when the user names one explicitly, and only
when that origin is self-hosted and under their control. When that happens, the
hosted rules below apply as written, with the self-hosted origin substituted for
`plan.agent-native.com` everywhere. Absent an explicit instruction naming an
origin, hosted publishing is out of scope for this fork — the sections below are
reference for that case, not the default path.

Two rules survive in every mode, and both are load-bearing:

- **Never fall back to an inline chat summary.** If the recap cannot be written
  or previewed, say what failed and stop. An inline Markdown or ASCII summary is
  the thing a recap replaces, not a degraded version of one.
- **Never transcribe secrets from a diff.** See the security section at the end
  of this file. That instruction is model-enforced, not tool-enforced, so treat a
  diff that touches `.env` or credential files as a reason to stop and ask.
...
## Top Canvas Recaps — read `references/canvas.md`

When a recap includes a top canvas, storyboard, or flow view, READ
`references/canvas.md` before authoring `canvas.mdx`. Recap
...
Upstream shipped this skill alongside `/visual-plan`, which is not part of this
plugin. The shared cores it depended on travel with this copy instead:
`references/wireframe.md` (wireframe and `<Screen>` quality),
`references/canvas.md` (canvas artboards and `canvas.mdx`), and
`references/local-files.md` (the local mode this fork defaults to). Read those
files here; do not go looking for a sibling skill directory.

- **check** — review-and-ship flow. A recap is the artifact a reviewer reads
  before `check` acts on the diff.
- **code-review-and-quality** — line-level assessment. The recap sets the shape;
  that skill judges the code.
- **agent-watchdog** — when the diff under recap was produced by another agent
  and the question is whether it did what was asked.
