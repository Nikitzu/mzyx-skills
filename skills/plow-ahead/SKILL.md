---
name: plow-ahead
description: "Explicit autonomy override: converts routine ambiguity into stated assumptions and keeps working to completion instead of stopping to ask, then recaps every decision made. Type it deliberately — it suspends the ask-first default and must never be self-invoked."
when_to_use: "plow ahead, do not stop, use your best judgment, keep going until done, finish while I am away, do not ask unless truly blocked"
...
## This Skill Is An Override — Know What It Suspends

Invoking this skill is an explicit instruction that outranks the ask-first
defaults in the user's global CLAUDE.md, specifically "Don't assume — surface
tradeoffs, unclarities, and missing context BEFORE coding" and the requirement to
check in when different readings would lead to materially different work. While
this skill is active, those become: decide, record the assumption, keep moving.

Three things do not bend, whatever the rest of this skill says:

- **The stop conditions below are hard stops.** Destructive, irreversible, or
  production-mutating actions, missing credentials, and branch or history
  operations the user did not request all still stop the work and ask.
- **The scope contract still holds.** Autonomy is permission to decide *how*, not
  licence to widen *what*. No unrequested ADRs, dev scripts, refactors of
  untouched code, or new files. Extras belong in a "Suggested, not done" list.
- **Verification still precedes any completion claim.** Autonomy raises the bar
  on evidence, because nobody was watching the work happen.

This skill is type-only (`disable-model-invocation: true`) by design. It must
never be auto-selected: an agent that grants itself permission to stop asking has
inverted the point of the skill.
